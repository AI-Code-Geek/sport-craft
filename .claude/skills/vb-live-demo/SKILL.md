---
name: vb-live-demo
description: Drives the SportCraft app (C:\mythigs\volleballTournament) through its full tournament lifecycle live in the browser — poll signup, captain selection, position categorization, live position-based auction bidding across multiple captain logins, schedule generation, live scoring, standings, and playoff bracket — while also demonstrating what each role (Super Admin, Org Admin, Captain, Player) sees. Use whenever asked to "demo", "walk through", "test", or "show" the app's workflow end to end.
---

# SportCraft — live UI walkthrough

This app has **no database — everything is Cloudflare KV** (see `docs/developer/02-data-model.md`).
Local dev persists KV to disk at `.wrangler/state/`, so state survives restarts. There is always one
**lazily seeded** organization ("Meridian Recreation Club") with one fully-played demo tournament
("Meridian Summer Volleyball League 2026" — poll closed, auction complete, 9/15 matches played).
**Never touch that seeded tournament during a demo** — re-saving its captains/positions/etc. can
regress its status or wipe its rosters (that bug is fixed, but the point of this skill is to demo the
*live* lifecycle, which the seeded tournament has already finished). Instead, **create a brand-new
tournament** for the walkthrough — creating one makes it the org's "active" tournament, so `/app/*`
pages immediately reflect it.
...
**Demo credentials — every account uses the same password `sportcraft2026`:**

| Role | Name | Email |
|---|---|---|
| Super Admin + Org Admin | Jordan Ellis | `admin@sportcraft.demo` |
| Org Admin | David Kim | `david.kim@sportcraft.demo` |
| Org Admin | Priya Nair | `priya.nair@sportcraft.demo` |
| Captain (after you assign them — pick any 6 from the confirmed pool) | e.g. Noor Santos | `noor.santos1@sportcraft.demo` |
| Player | any other confirmed voter | see `/app/admin/users` as an Org Admin for the full list |
...
Open a tab, navigate to `http://localhost:3100/`, screenshot the landing page (Log in / Join a
community / Request an org tabs, pre-filled Super Admin demo credentials).
...
const res = await fetch('/api/auth/login', {
  ...
  body: JSON.stringify({email:'<email>', password:'sportcraft2026'})
});
(await res.json()).user?.name
```

- **Super Admin** (`admin@sportcraft.demo`) — nav shows an "Admin" link AND an "Organizations" link
  (platform-wide org-request review + org list, Super-Admin-only); the Nav badge reads "Super Admin";
  `/app/admin` shows the full org-admin tool grid (this account is also this org's Org Admin).
- **Org Admin** (`david.kim@sportcraft.demo`) — nav badge reads "Org Admin"; `/app/admin` shows the
  same operational tools (new tournament, poll, captains, positions, auction, schedule, scoring,
  playoffs, users, org settings) but NO "Organizations" link — that's platform-level, not this org's.
- **Captain** (any team's captain, once assigned — see §3) — nav badge reads "Captain" (amber); no
  Admin link; `/app/auction` shows a live bid panel instead of a read-only view when it's their team's
  turn.
- **Player** (any confirmed non-captain) — no badges at all; fully participant-only nav (Home, Poll,
  Teams, My Team, Auction, Schedule, Standings, Bracket) — filtered further by whichever features this
  org has enabled (`/app/admin/settings`).

## 2. Create a fresh tournament (Org Admin or Super Admin — both can now)

Log in as `admin@sportcraft.demo`. Navigate to `/app/admin/tournaments/new`. The form is pre-filled
with sane defaults (6 teams, team size 6, 100 auction points, best-of-3 to 25, playoff cutoff 4) —
click **Create tournament**. This immediately becomes the org's active tournament. Screenshot the
admin console (`/app/admin`) — the lifecycle stepper now shows "Poll" as current, everything else
greyed out.
...
const community = await (await fetch('/api/community/users')).json();  // { members, pending }
const staff = ['admin@sportcraft.demo','david.kim@sportcraft.demo','priya.nair@sportcraft.demo'];
const voters = community.members.filter(u => !staff.includes(u.email)).slice(0, 36);
...
- **Step 1 "Name your teams"** — rename the 6 default team cards (e.g. Meridian Smashers / Net
...
`/app/admin/auction` (org admin control) and `/app/auction` (captain/spectator view) both show the
same live state, polling every 3s.

1. Screenshot `/app/auction` as Super Admin first — read-only, "No bids yet — first eligible bid
   wins the floor."
2. `javascript_tool`-login as **captain #1** (e.g. `noor.santos1@sportcraft.demo`), reload
...
5. `javascript_tool`-login as **Priya Nair** (`priya.nair@sportcraft.demo`, an Org Admin of this org),
   navigate `/app/admin/auction`, click **Mark sold →** — screenshot the budget rail updating (slot
   filled, points deducted) and the board advancing to the next nominee.
...
- `/app/admin/schedule` — form is pre-filled (a generic "Community Gym" default, Court A/Court B,
  18:00 & 19:15, a start date, 7 days between rounds — edit the venue name if you want it to match
  your demo org). Click **Generate & publish schedule** — screenshot the 15-match round robin that
  appears (3 matches/round × 5 rounds, no double-booked court+time).
